...
public class myagent {
    ...
    public static myagent dummy;
    public static boolean gameTerminate = false;
    ...
    public static class ABNode {
        ...
        int depth;
        String pos;
        List<String> TraverseNode = new ArrayList<String>();
        ...
        ABNode(ABNode c) {
            ...
            this.value = c.value;
            this.oper = c.oper;
            this.TraverseNode=c.TraverseNode;
        }
        ...
        try {
            if (args.length > 0) {
                ...
                try {
                    ...
                    PrintStream ps = new PrintStream(new BufferedOutputStream(new java.io.FileOutputStream(f)), true);
                    System.setOut(ps);
                } catch (Exception e) {
            ...
            if (Task == 4) {
                try {
                    ...
                    if (CutOffDepth > 100) {
                        CutOffDepth = 8;
                    } else if (CutOffDepth > 60) {
                        CutOffDepth = 8;
                    }else if (CutOffDepth > 30) {
                        CutOffDepth = 3;
                    } else if (CutOffDepth < 10) {
                        CutOffDepth = 2;
                    }
                    ...
                    if (Task == 3) {
                        try {
                            ...
                            ABNode b = alphaBeta_decision(Player, 0, Board);
                            Board = b.board;
                            println("===========\nbest board\n");
                            List<String> str = printBoard();
                            str=b.TraverseNode;
                            writeTextFile("output.txt",str );
                        } catch (Exception e) {
                    ...
                    System.out.println("error task 4");
                }
            }
    ...
    public static void writeTraverseFile(String aFileName, String line, boolean createFile) {
        Path path = Paths.get(aFileName);
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter(aFileName, createFile));
            {
...
//                                        , true);
                                mNode.depth = depth + 1;
                                mNode.TraverseNode = new ArrayList<>();
                                mNode.TraverseNode.add(0, node + pos);
                                for (int kk = 0; kk < mnodeChild.TraverseNode.size(); kk++) {
                                    mNode.TraverseNode.add(kk + 1, mnodeChild.TraverseNode.get(kk));
                                }
...
//                        , true);
    ...
    public static ABNode alphaBeta_decision(int Player, int depth, int[][] Board) {
        int dummyBoard[][] = initDummy(Board);
        ...
        int alpha = -INFINITY;
        int beta = INFINITY;
        ABNode bestBoard = new ABNode(alpha, beta, dummyBoard, oper, maxValue, depth);
        ...
        for (int k = 1; k < BoardSize + 1; k++) {
            ...
            if (dummyBoard[Player - 1][k] > 0) {
                ...
                if (values[k - 1] > maxValue) {
                    maxValue = values[k - 1];
                    maxPos = k;
                    bestBoard = new ABNode(mNodeRet);//initDummy(mNodeRet.board);
                    bestBoard.pos=node+pos;
